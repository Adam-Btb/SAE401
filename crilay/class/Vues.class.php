...
class Vues
{
    ...
    public function afficherFeed($publications, $nouveaux, $eventsPublics)
    {
        ...
        echo '<form method="POST" action="index.php?action=publier" enctype="multipart/form-data" class="post-form" style="display:flex; flex-direction:column; gap:12px;">';
        echo '<textarea name="contenu" placeholder="' . $this->t('write_post') . '" rows="2" style="margin-bottom:0;"></textarea>';
        echo '<div style="display:flex; justify-content:space-between; align-items:center;">';
        echo '<input type="file" name="image" accept="image/*" style="font-size:14px; max-width:200px;">';
        ...
        foreach ($publications as $p) {
            ...
            if (!empty($p['image'])) {
                echo '<img src="img/posts/' . $this->esc($p['image']) . '" alt="" style="max-width:100%; border-radius:10px; margin-top:12px; border:1px solid rgba(100,100,100,0.2);">';
            }
            echo '</div>';
        }
        echo '</div>';
