...
class Utilisateur {
    ...
    public function publier($userId, $contenu, $imageFile = null) {
        $contenu = trim($contenu);
        if (empty($contenu) && empty($imageFile['name'])) return false;
        
        $imageName = null;
        if (!empty($imageFile['name']) && $imageFile['error'] === UPLOAD_ERR_OK) {
            $ext = strtolower(pathinfo($imageFile['name'], PATHINFO_EXTENSION));
            if (in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
                $imageName = uniqid('post_') . '.' . $ext;
                if (!is_dir('img/posts')) @mkdir('img/posts', 0777, true);
                move_uploaded_file($imageFile['tmp_name'], 'img/posts/' . $imageName);
            }
        }
        $req = $this->cnx->prepare("INSERT INTO publications (auteur_id, contenu, image) VALUES (:uid, :contenu, :image)");
        $req->execute([':uid' => (int)$userId, ':contenu' => htmlspecialchars($contenu), ':image' => $imageName]);
        return true;
    }
}
