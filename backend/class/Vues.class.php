<?php
class Vues {
    private function t($k) { return Langue::t($k); }
    private function lang() { return Langue::getLang(); }
    private function esc($s) { return htmlspecialchars($s, ENT_QUOTES, 'UTF-8'); }
    ...
    public function debutHtml($titre, $connecte = false) {
        ...
        if ($connecte) {
            $navItems = [
                ...
                ['action' => 'amis',      'label' => $this->t('friends'),  'icon' => '&#128101;'],
                ['action' => 'profil',    'label' => $this->t('nav_profile'), 'icon' => '&#128100;'],
            ];
            ...
            $pubItems = [
                ['action' => 'apropos', 'label' => $this->t('nav_about'), 'icon' => '&#8505;'],
                ['action' => 'equipe',  'label' => $this->t('team'),      'icon' => '&#128101;'],
                ['action' => 'faq',     'label' => $this->t('faq'),       'icon' => '&#10068;'],
            ];
        ...
        echo '</div>'; // topnav-actions

        echo '</div></header>'; // topnav
    ...
    public function afficherAccueil() {
        ...
        // Vidéo
        echo '<section class="section-video">';
        echo '<h2>' . $this->t('discover_video') . '</h2>';
    ...
    public function afficherInscription($tags, $erreur = '', $success = '') {
        ...
        if ($erreur) echo '<div class="alert alert-error">' . $this->t($erreur) . '</div>';
        if ($success) echo '<div class="alert alert-success">' . $this->t($success) . '</div>';
    ...
    public function afficherProfil($user, $tags, $estMoi = false, $relationAmitie = null) {
        ...
        if ($age !== null) echo '<p>' . $age . ' ' . $this->t('years_old') . '</p>';
        if (!empty($user['ville'])) echo '<p>' . $this->esc($user['ville']) . '</p>';
        if (!empty($user['nationalite'])) echo '<p>' . $this->esc($user['nationalite']) . '</p>';
        if (!empty($user['langues_parlees'])) echo '<p>' . $this->esc($user['langues_parlees']) . '</p>';
    ...
    public function afficherRecherche($resultats, $tags, $filtres) {
        ...
        if (empty($resultats)) {
            ...
            foreach ($resultats as $u) {
                ...
                if (!empty($u['ville'])) echo '<p class="user-card-city">' . $this->esc($u['ville']) . '</p>';
                if (!empty($u['tags_noms'])) echo '<p class="user-card-tags">' . $this->esc($u['tags_noms']) . '</p>';
        ...
        }, array_filter($resultats, function($u) { return !empty($u['latitude']); }))) . ';';
    ...
    public function afficherConversations($conversations) {
        ...
        if (empty($conversations)) {
            ...
            foreach ($conversations as $c) {
                ...
                if ($c['non_lus'] > 0) echo ' <span class="badge-notif">' . $c['non_lus'] . '</span>';
    ...
    public function afficherConversation($messages, $contact, $amiObj, $userId, $infoMsg = '') {
        ...
        foreach ($messages as $m) {
            ...
            if ($m['est_invitation']) echo '<div class="msg-invitation-badge">&#9733;</div>';
        ...
        if (!$sontAmis) {
            $dejaEnvoye = false;
            foreach ($messages as $m) {
                if ($m['expediteur_id'] == $userId) { $dejaEnvoye = true; break; }
            }
            $peutEcrire = !$dejaEnvoye;
        }

        if ($peutEcrire) {
            echo '<form method="POST" action="index.php?action=envoyer_msg&dest=' . $contact['id'] . '" class="chat-form">';
            echo '<textarea name="contenu" placeholder="' . $this->t('type_message') . '" required></textarea>';
    ...
    public function afficherAgenda($evenements, $invitations, $amis) {
        ...
        foreach ($evenements as $e) {
            ...
            if ($e['type'] !== 'prive' || ($e['role'] ?? '') === 'organisateur') {
                ...
                if (!empty($e['description'])) echo '<p class="event-desc">' . $this->esc($e['description']) . '</p>';
            }
    ...
    public function afficherFeed($publications, $nouveaux, $eventsPublics) {
        ...
        // Formulaire
        echo '<form method="POST" action="index.php?action=publier" class="post-form">';
        echo '<textarea name="contenu" placeholder="' . $this->t('write_post') . '" required rows="2"></textarea>';
        ...
        echo '</div>'; // feed-sidebar
        echo '</div>'; // feed-layout
        echo '</section>';
    }
