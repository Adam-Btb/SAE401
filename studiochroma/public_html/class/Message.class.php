...
class Message {
    ...
    public function envoyer($expediteurId, $destinataireId, $contenu, $ami, $fileData = null) {
        $sontAmis = $ami->sontAmis($expediteurId, $destinataireId);
        $estInvitation = 0;

        if (!$sontAmis) {
            $req = $this->cnx->prepare("SELECT COUNT(*) as nb FROM messages WHERE expediteur_id = :exp AND destinataire_id = :dest");
        ...
        $sql = "INSERT INTO messages (expediteur_id, destinataire_id, contenu, piece_jointe, est_invitation) VALUES (:exp, :dest, :msg, :pj, :inv)";
    ...
    public function compterNonLus($userId) {
        $req = $this->cnx->prepare("SELECT COUNT(DISTINCT expediteur_id) AS nb FROM messages WHERE destinataire_id = :uid AND lu = 0");
